...
def compute_load(
    dataframe, col_begin, col_end, col_cumsum, begin_time=0, end_time=None
):
    """
    ...
    start_event_df = pd.concat([df[col_begin], df[col_cumsum], df["jobID"]], axis=1)
    start_event_df.columns = event_columns
    # Stop event give negative proc_alloc value
    stop_event_df = pd.concat([df[col_end], -df[col_cumsum], df["jobID"]], axis=1)
    ...
    # event_df = (
    #     pd.concat([start_event_df, stop_event_df], axis=1, ignore_index=True)
    #     .sort_values(by="time")
    #     .reset_index(drop=True)
    # )
...
def _load_insert_element_if_necessary(load_df, at):
    ...
    if len(load_df[load_df.time == at]) == 0:
        ...
        load_df.loc[prev_el.index, "area"] = float(prev_el.load) * float(
            at - prev_el.time
        )
